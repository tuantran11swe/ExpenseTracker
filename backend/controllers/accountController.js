...
export const createUserAccount = async (req, res) => {
  try {
    ...
    const userAccountNames = Array.isArray(name) ? name : [name];
    ...
    const updateUserAccountQuery = {
      ...
      values: [userAccountNames, userId],
    };
    await pool.query(updateUserAccountQuery);

    // Tạo transaction record cho khoản tiền gửi ban đầu
    const transactionDescription = `${newAccount.account_name} - Tiền gửi ban đầu`;
    ...
    res.status(201).json({
      data: newAccount,
      message: `Tài khoản ${newAccount.account_name} đã được tạo thành công`,
      status: "success",
    });
  } catch (error) {
    // Xử lý lỗi nếu có bất kỳ exception nào xảy ra
...
export const depositToAccount = async (req, res) => {
  try {
    ...
    const queryResult = await pool.query({
      ...
      values: [depositAmount, id],
    });

    const accountInformation = queryResult.rows[0];

    // Tạo mô tả cho transaction
    const transactionDescription = `${accountInformation.account_name} - Nạp tiền`;
